...
class ConfirmDialog:
    ...
    def __init__(
        ...
        self.dialog = Adw.MessageDialog(transient_for=parent, heading=heading, body=body)
        self.dialog.add_response('cancel', cancel_label)
        self.dialog.add_response('confirm', confirm_label)

        if destructive:
            self.dialog.set_response_appearance('confirm', Adw.ResponseAppearance.DESTRUCTIVE)
        else:
...
class IconPickerDialog(Adw.Window):
    ...
    def _build_ui(self):
        ...
        self.emoji_picker = EmojiPicker(
            selected_emoji=self.selected_icon if self.selected_type == 'emoji' else '', on_change=self._on_emoji_change
        )
