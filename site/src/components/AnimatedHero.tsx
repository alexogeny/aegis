...
export function AnimatedHero() {
  ...
  return (
    <div ref={containerRef} class="relative">
      {/* Animated tagline */}
      <h1 class={`text-5xl md:text-7xl font-bold text-text mb-6 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
      ...
      <p class={`text-lg md:text-xl text-subtext0 max-w-2xl mx-auto mb-8 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
      ...
      <div class={`flex flex-col sm:flex-row items-center justify-center gap-4 transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        <a
          href="/download"
        ...
        >
          ...
          <svg class="w-4 h-4 opacity-50 group-hover:opacity-100 group-hover:translate-x-1 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      {/* Floating badges */}
      <div class={`mt-12 flex flex-wrap items-center justify-center gap-4 transition-all duration-1000 delay-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
...
function Badge({ icon, text, color, delay }: { icon: string; text: string; color: string; delay: number }) {
  const [isVisible, setIsVisible] = useState(false);
