...
const DEMO_COMMANDS: Command[] = [
  ...
  {
    cmd: 'cargo build',
    output: ['   Compiling aegis-project v0.1.0', '    Finished dev [unoptimized + debuginfo]'],
  },
];
...
export function InteractiveTerminal() {
  const [phase, setPhase] = useState<Phase>({ type: 'typing', cmdIndex: 0, charIndex: 0 });
  const [history, setHistory] = useState<Array<{ type: 'cmd' | 'output'; content: string }>>([]);
    ...
    if (phase.type === 'typing') {
      ...
      if (phase.charIndex <= cmd.cmd.length) {
        ...
        timer = window.setTimeout(() => {
          setPhase({ type: 'typing', cmdIndex: phase.cmdIndex, charIndex: phase.charIndex + 1 });
        }, 50 + Math.random() * 50);
      } else {
        // Done typing, execute
        timer = window.setTimeout(() => {
          ...
          setCurrentInput('');
          setPhase({ type: 'executing', cmdIndex: phase.cmdIndex, outputIndex: 0 });
        }, 300);
      }
      ...
      if (phase.outputIndex < cmd.output.length) {
        timer = window.setTimeout(() => {
          setHistory((h) => [...h, { type: 'output', content: cmd.output[phase.outputIndex] }]);
          setPhase({ type: 'executing', cmdIndex: phase.cmdIndex, outputIndex: phase.outputIndex + 1 });
        }, 80);
      } else {
        // Done with output
  ...
  return (
    <div class="bg-base rounded-xl border-2 border-surface0 overflow-hidden shadow-2xl">
      ...
      <div class="bg-crust px-4 py-2 flex items-center justify-between">
        ...
        <span class="text-[10px] text-overlay0 bg-surface0 px-2 py-0.5 rounded">Demo</span>
      </div>
      ...
      <div ref={terminalRef} class="p-4 font-mono text-sm h-64 overflow-y-auto">
        ...
        <div class="flex items-center flex-wrap mb-2">
          <span class="bg-mauve text-crust px-2 py-0.5 text-xs font-bold"> aegis</span>
          <span class="text-mauve text-xs"></span>
          <span class="bg-peach text-crust px-2 py-0.5 text-xs font-bold"> ~/dev</span>
          <span class="text-peach text-xs"></span>
          <span class="bg-yellow text-crust px-2 py-0.5 text-xs font-bold"> main</span>
          <span class="text-yellow text-xs"></span>
          <span class="bg-teal text-crust px-2 py-0.5 text-xs font-bold"> </span>
        </div>
          ...
          <div key={i} class={entry.type === 'cmd' ? 'mt-3' : 'text-xs ml-2'}>
            {entry.type === 'cmd' ? (
              <div class="text-text">
                ...
                <span class="text-blue"> {entry.content.split(' ').slice(1).join(' ')}</span>
              </div>
            ) : (
        ...
        <div class="text-text mt-3">
          ...
          <span class="text-blue"> {currentInput.split(' ').slice(1).join(' ')}</span>
